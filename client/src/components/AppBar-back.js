...
function ResponsiveAppBar() {
  const [socket, setSocket] = React.useState(null); // Socket connection
  const { setUser, user } = React.useContext(AuthContext);
  const [anchorElNav, setAnchorElNav] = React.useState(null);
  const [anchorElUser, setAnchorElUser] = React.useState(null);
  const [messageCount, setMessageCount] = React.useState(5); // Example unread messages
  ...
  React.useEffect(() => {
    ...
    // Establish socket connection
      ...
      //  newSocket.on('activeChats');
      //  newSocket.on('inactiveChats');
      ...
      const refreshChat = () => {
        ...
        setMessageCount((prevMessageCount) => {
          let newMessageCount = 0;
            ...
            newMessageCount = (prevMessageCount || 0) + 1;
          console.log(newMessageCount)
          return newMessageCount; 
        });
      }
    
      console.log("test", messageCount)
      socket.on('messageCount', refreshChat);

      
    
      return () => {
        socket.off('messageCount', refreshChat);
      };
    }, [socket]);
